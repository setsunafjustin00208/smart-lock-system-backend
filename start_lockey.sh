...
# Split horizontally → Pane 2: Hardware Log Monitor
/usr/bin/tmux split-window -h -t $SESSION -c $WORKDIR
/usr/bin/tmux send-keys -t $SESSION "while true; do php spark hardware:logs; echo 'Log monitor stopped. Restarting in 3s...'; sleep 3; done" C-m
...
echo "  php spark hardware:logs    - Monitor hardware activity logs"
echo "  php spark hardware:monitor - Monitor device status"
echo "  tmux attach -t $SESSION    - Attach to main session"
